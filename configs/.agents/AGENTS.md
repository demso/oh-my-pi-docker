...
Follow the repository's existing commit style. For Pallas (both `pallas-web` and `ks-sechero-app`): imperative, capitalized, no prefixes — "Add X", "Fix Y", "Update Z", "Remove W". No Conventional Commits (`feat:`/`fix:`/etc.). Check the repo's recent `git log` to match its style when in doubt.
